...
- **clients** (existing) — id, slug (unique), business_name, address, phone_display, email, license_number, hours (jsonb), logo_url, brand_color (default `#bd703e`), service_area (text[]), tagline, review_place_id, review_link, google_review_toggle (enum `review_gate_mode`, default `gated`), star_threshold (smallint, default 4), twilio_number, twilio_messaging_service_sid, sending_subdomain, dkim_status (both **[DEFERRED — not used in v1]**: one platform-level email sender, not per-client domains), status (default `active`), template_vars (jsonb), created_at, updated_at.
