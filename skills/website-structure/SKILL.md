...
### Site-wide conversion elements [added 2026-07-27]
...
- **[LAUNCH PREREQUISITE — 1f step 3 dependency] Every public lead-intake form (website lead form, discount-claim, AND the chat-widget opt-in) MUST run the INVISIBLE native PoW bot-shield (challenge from `/api/public/challenge`, solved in a Web Worker) and submit `pow_token` + the hidden `website` honeypot field in the POST body (2026-07-22 — replaces the visible Turnstile widget; legacy `turnstile_token` still verified server-side).** The backend enforces the bot check on `intake`/`discount`/`chat/optin` (fail-closed on an invalid token), so a form WITHOUT the shield → no token → **every legit submission is rejected → zero leads.** (No hostname registry, no site keys — the Turnstile widget + hostname step are RETIRED 2026-07-22; historical build-spec `docs/1f-step3-turnstile-ratelimit-build-spec.md`; gated in `/launch-check` §E.)
- **[A2P-PREP — compliance pages] Every site carries the carrier-compliance surface from `/a2p-site-compliance`:** the single-checkbox opt-in (marketing skeleton, unchecked/optional on the lead form; fixed consent skeleton + per-niche category strings keyed by `{segment}`), the named (not generic) Privacy Policy + Terms of Service, the SMS Program page, footer Privacy/Terms/SMS-Program links on every page, and the working `{site_url}/review` page. **Copy is reproduced VERBATIM from the `/a2p-site-compliance` skill's "Appendix — Canonical Verbatim Copy" — tokens only, never paraphrased** (the same copy is also kept at `docs/a2p-compliance-copy-source-of-truth.md` for human reference + parity). (Carrier 10DLC review reads the live site.) These pages + the invisible native PoW bot-shield (Turnstile RETIRED 2026-07-22) are **baked into the STYLE template** (`/template-builder`) once, so every per-client remix inherits them — not authored per client.
