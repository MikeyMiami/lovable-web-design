...
- **Dynamic render routes** the marketing template builds ONCE: `/` (home), `/services` (index), `/services/$slug` (service + category), `/service-area/$slug` (geo/neighborhood), `/$slug` (supporting), a **Locations index** (links to all geo pages), and a **sitemap** generated from the store. Each route renders a `content_pages` row and applies **§1/§2/§3** (schema, meta, semantic HTML, editorial internal links, breadcrumbs). `/$slug` is **lowest routing precedence** — renders `type='supporting'` only, 404 otherwise; it must never shadow fixed routes (about/contact/gallery/compliance/funnel); collisions are rejected at write time.
- **Anon-RPC call contract [LOCKED — templates MUST match exactly]:** the render routes read via three SECURITY DEFINER RPCs whose **named args are `_slug`-prefixed** — `get_client_public(_slug)`, `get_client_pages(_slug)`, `get_client_page(_slug, _page_slug)`. PostgREST resolves RPCs by **named argument**, so a wrong param name (e.g. `p_client_slug`/`p_page_slug`) **404s silently** — and a template that carries a demo fixture will fall back to it and *look* like it works. Two rules: (1) call the RPCs with the **exact** `_slug` / `_page_slug` names; (2) the returned `content_pages` row's page identifier column is **`slug`** — map it to whatever the template calls it (e.g. `page_slug`) at the row-shape boundary. **Always surface RPC errors (log/throw) — never silently fall back to a fixture**, or a param/shape mismatch hides as a working page.
