...
> **⚠️ FROZEN LEGACY (2026-07) — TextGrid is no longer the default.** **Telnyx** (see `/telnyx-provider`) is now the go-forward DEFAULT and the ONLY provider selectable for new clients: `clients.provider` now defaults to `'telnyx'` (flipped from `'textgrid'`); the admin Messaging Provider select was RETIRED 2026-07-22 (card deleted — flipping a legacy client is SQL-only). **TextGrid is FROZEN LEGACY — it still routes any client already on `'textgrid'` (its four edge functions, send transport `sendViaTextGrid`, and behavior are all intact and byte-frozen), but it is not the default and cannot be chosen for new clients.** Any change to the frozen path during the wind-down is an explicit, logged exception. Shared webhook business logic lives in `supabase/functions/_shared/core.ts` (written to byte-match these frozen functions); bugs found there are fixed in `_shared/` only (Telnyx path), divergence noted, never back-ported silently. Root cause of building Telnyx: TextGrid's **confirmed caller-side ringback defect** (all four `<Dial>` ringback attributes ignored on genuinely-served TwiML — `docs/VOICE-RINGBACK-TEST-RESULTS.md`). Endgame + retirement quiet-period: `/telnyx-provider` §9.
...
- `from`-resolution at the caller (see §2): the primitive takes `{clientId, contactId, body}` with NO `from`. At 1f the CALLER resolves + passes `from` (= `clients.twilio_number`) + per-client auth, so the primitive STAYS SEND-ONLY (no DB reads inside it). This is the exact from-resolution seam reused for every per-client send — including reactivation, which (since the agency number pool was **REMOVED 2026-07-21**) now sends from the client's OWN number via the normal runner, never a pool number.
...
**`from`-resolution [the reusable seam]:** the primitive today takes `{clientId, contactId, body}` — NO `from` argument. At 1f, change the arg-shape so the CALLER resolves and passes `from` (= `clients.twilio_number` for per-client sends) + per-client auth. This keeps the primitive SEND-ONLY (it does not read the DB to find the number). **Reactivation reuses this SAME seam** — but since the agency number pool was **REMOVED (2026-07-21)** it sends from the client's OWN number (`clients.telnyx_number` on the go-forward path) via the normal runner, not a separate pool runner/number. Design the `from`+auth arg-shape ONCE so every per-client send passes it — no further frozen change needed.
...
> **✅ LIVE-VALIDATED 2026-07-11** (reproduced a real inbound signature from a captured POST): the signing key = the per-client `provider_webhook_secret` (confirmed — NOT the account/subaccount AuthToken), and the signed message = `request.url + raw body` bytes VERBATIM (NOT Twilio's sorted-params scheme). `request.url` == the configured `smsUrl` exactly on Lovable/Cloudflare. The original "replies don't show" failure was a silent fail-closed 401 because the secret wasn't yet stored. See `docs/build-log/1f-inbound-webhook-live-validation.md`. **[STORAGE MOVED 2026-07-22]:** the per-client webhook secret + subaccount SID/auth token now live in the server-only **`client_provider_secrets`** table (clients columns dropped — security hardening); edge fns + server code read it service-role; admin sets values via the write-only ProviderSecretsPanel.
>
> **⚠️ Diagnostics caveat — TextGrid REST READ APIs are UNRELIABLE.** `GET /Messages.json` returns empty even for messages visible in the dashboard UI; `/Notifications.json` and `/Accounts.json` (subaccounts) return `404`. Do NOT diagnose delivery via these. The dashboard UI is the source of truth for message existence; to inspect a real inbound POST (URL/headers/signature/body), temporarily repoint the number's `smsUrl` to a **webhook.site** URL, text the number, then restore. TextGrid API auth works with HTTP Basic (`-u AccountSid:AuthToken`) or `Bearer base64(sid:token)`; set number config via `POST /2010-04-01/Accounts/{sid}/IncomingPhoneNumbers/{PNsid}.json`.
...
2. **Create Brand (non-blocking):** `POST /campaigns/brand/nonblocking` with `subAccountSID = <client subaccount sid>`, `entityType: "PRIVATE_PROFIT"` (typical local business; SOLE_PROPRIETOR path needs SMS OTP — see below), client's `companyName`/`ein`/`einIssuingCountry`/**legal business address** (`clients.legal_address` — the registered address from the client's IRS documents; may differ from the public site address)/`phone`/`email`/`website`/`vertical`/`brandRelationship`. Returns `brandId` (B-prefixed), `identityStatus: PENDING`. Callback `BRAND_IDENTITY_STATUS_UPDATE` → VERIFIED/UNVERIFIED.
...
4. **Create Campaign:** `POST /campaigns/campaign` with `brandId`, `usecase`, `description` (must include "message frequency may vary"), `subscriberOptin/Optout/Help: true`, `sample1..5` (real sample drip messages), `messageFlow` (the opt-in flow text), `helpKeywords/helpMessage`, `optinKeywords/optinMessage`, `optoutKeywords/optoutMessage`, `termsAndConditionsLink`, `privacyPolicyLink`, `embeddedLink: true` (your review links). Returns `campaignId` (C-prefixed). Optional `expediteCampaign: true` (+$27). Note the DCA2 $15 manual-vet per submission.
   - **TCR Nov-2024 content rules (campaign WILL be rejected without these):** privacy policy must state mobile opt-in is NOT shared with third parties for marketing; ToS must carry the SMS disclosure (message types, cadence, msg&data-rates, privacy link, opt-out); opt-in CTA must show brand + msg-frequency + fees + HELP/STOP; the phone field on the opt-in site must be OPTIONAL (a required phone = "forced opt-in" = rejection). **These are satisfied by the client's marketing site — see §6 handoff to the A2P-site-compliance skill.** The Brand fields, Campaign Description, Call-to-Action, sample messages, and `termsAndConditionsLink`/`privacyPolicyLink` submitted in this step are **PRODUCED by `/a2p-site-compliance`** (admin A2P-prep panel; verbatim copy `docs/a2p-compliance-copy-source-of-truth.md`) — do not re-author registration copy here; consume the generated pack.
