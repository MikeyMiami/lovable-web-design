...
- **Homepage** — talks directly to the searcher (what you do, how fast, where). The intro is a **customer-facing pitch answering "can this business do what I need?"** — lead with **capability + outcomes** (breadth of what they handle, ability/speed to deliver) and **why-choose-us grounded in the client's `differentiators`** (woven naturally, never invented — the accuracy guard applies; mention local track record only if provided), NOT generic company self-description — **then** route to the categories. Each secondary category gets an H2 + **300–400 words** + an editorial link to its category page (`40-page:119-120`). *(H1/title = primary category + city — `40-page:102-108`.)*
...
- **Service page** — specific to that one service: why customers need it, what's included, how long, what to expect, **pricing ranges where available** (see note), areas you serve for this service. End with a CTA. **800–1500 words** (`40-page:156-161`). Not generic advice.

> **[Word-count note — method target vs first pass]** The numbers above (home 300–400/secondary-category section, service **800–1500** words) are the EXPERT METHOD'S targets — hit by the full **content-automation tool** (§4 8-pass + §5 research, which produces enough genuine local substance to fill that length). The **current single-pass `aiWritePage`** is the **lighter first draft** and deliberately runs shorter (~400–700 words) to avoid padding/AI-slop — do NOT bump its target without the research/8-pass depth to back it. The tool closes the gap to the method length.
> **[Pricing-range note]** Service pages want pricing (even ranges) per the method (`40-page:159`). This requires **CAPTURING real ranges from the client** (onboarding / SEO panel) and feeding them as PROVIDED CONTEXT — the anti-hallucination guard (§1b) stays: the writer **never invents pricing.** Until ranges are captured, the writer correctly omits pricing.
...
## 6. Linking — THREE distinct link types [LOCKED — do not conflate]
The expert's build pipeline names them as separate steps (`W3deWbeigsg:140-149`: "Internal links get placed… An outbound link gets placed… we source an external link from another website pointing back to that article"):
1. **Internal (in-content, to OUR pages) — AI-woven, on-page.** Editorial links following the seo-build hierarchy (home→category→service; supporting/geo pages link to their Core-30 parent; a Locations index links to all geo pages). Nav/footer links don't count. **FAQ pattern:** question on the relevant page → answer briefly → editorial link to a deep supporting page.
2. **Outbound (in-content, to an AUTHORITY source) — AI-woven, on-page, ~1/page.** One natural in-content link out to a relevant authority source (industry body, .gov/.edu, standards/manufacturer page — not a competitor). **Operator-provided URL** (stored in `content_pages.external_link`); the writer weaves ONE natural sentence linking to it. **The AI must NEVER invent the URL** — a fabricated citation is a hallucination; if none is provided, no outbound link is written. (`FifSqbB0:534-535` "inserting highly relevant external links to authority sources.")
3. **Inbound / backlinks (on OTHER sites, pointing TO our page) — OFF-SITE, agency-acquired, NOT a page feature.** The expert's "every page needs at least one quality external link **pointing to it**… not optional" (`40-page:195-200`) + "not AI slop links… other sites trust your content enough to **link to it**… one per page… 30 external links for the core 30" (`ZKZnDORR0ds:228-232`). These live on OTHER websites — **nothing is stored on our row and the AI writes nothing.** Acquired off-site: **chamber-of-commerce memberships** (~$200-300/yr; "a link from their website back to yours", `ZKZnDORR0ds:242-243`), **paid "not-AI-slop" links** (~$25-35 each, one per Core-30 + supporting + geo page, `ZKZnDORR0ds:348-350,383-384`), local sponsorships. **Mandatory per the method** (Google uses them as a quality-verification signal, especially for AI content) but **agency-ops, off-site — see the Agency Link-Building Tracker (roadmap), NOT the page builder.**
