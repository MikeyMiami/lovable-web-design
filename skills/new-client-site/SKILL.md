...
- **[BUILT — C-3c-1; SPLIT 2026-07-23] Finalize & Activate (per-client `/admin/review`):** once config is set, mint the client login via **`provisionClientOwner`** (SILENT `createUser` → the business notification email = `notification_email ?? email`; audited `client_owner` grant; flips `pending`→`active`) and read the **Remix handoff checklist** there (slug, `VITE_CLIENT_SLUG`, `allowed_origins`) — which feeds step 3 (Remix). **NO email is sent at this step.** Manual + idempotent. The client's welcome email (**Send Client Welcome**, with their website / set-password / app-download links) is a SEPARATE action fired at LAUNCH once the site is live — it is code-gated on `template_vars.company_website_link` being set. Detail: `/admin-view` → Finalize & Activate + Send Client Welcome.
...
- Write the provisioned number to `clients.twilio_number` (**TextGrid path only** — single-source; the unchanged runner picks it up). **Telnyx fork:** write `telnyx_number` + `telnyx_brand_id` + `telnyx_campaign_id` — **those THREE are the only per-client Telnyx IDs (2026-07-29).** `telnyx_messaging_profile_id`/`telnyx_texml_app_id` are NOT per-client and must NOT be written: the messaging profile and TeXML voice app are **SHARED platform-wide** (one profile + one voice app, every client number on both) and are never read at runtime — sends use the `from` number, inbound resolves by the `to` number. Routing keys off `clients.provider`, NOT `twilio_number`. Place the active number on the site + Google Business Profile.
