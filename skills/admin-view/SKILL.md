...
- **Messaging config** — twilio_number, twilio_messaging_service_sid, call_forwarding_number (the client's real phone the Twilio number rings through to; editable per client over time), sending_subdomain, dkim_status. These per-client Twilio values are NON-secret (From + Messaging Service SID, under ONE parent account). The Twilio auth token is a single platform runtime secret, never per client. (BYO-Twilio per-client creds = Option 2, future, in a server-only `client_secrets` store — not here.)
