...
- [ ] Runtime secrets set: CRON_SECRET, parent messaging-provider (TextGrid) auth token (NOT on any row).
...
## D. Telephony (messaging provider = TextGrid; Twilio-API-compatible)
- [ ] One parent messaging-provider (TextGrid) account (fetch, no SDK); per-client subaccount → Brand → Campaign → number; From/MessagingServiceSid on the clients row (column names retained).
...
- [ ] **[1f — NET-NEW, not a swap]** Inbound + voice-status webhooks (provider = TextGrid) under `/api/public/*`; route by To → clients row; **`X-TextGrid-Signature` (HMAC-SHA1)** verified BEFORE any DB write. *(No inbound route or signature check exists in the frozen master — built fresh at 1f.)*
- [ ] STOP/HELP/START + "pass" opt-out handling wired.
- [ ] **The 1f net-new inbound webhook (TextGrid) drives REAL-TIME drip exits** (not deferred to the next pre-step check): one-year reply → exit + interest notification immediately (gaps are weeks/months — pre-step-only would delay the notification ~a month); missed-call reply-skip; opt-out. (Pre-step checking is the stub-mode fallback only.)
...
- [ ] **GATE — Stage 1f shipped (the deliberate FINAL pre-launch hardening, AFTER the Stage-4 freeze):** real messaging-provider (TextGrid) swap + the net-new inbound webhook layer + message testing + LIVE Turnstile + rate-limiting on all public lead-intake routes before this client goes public — incl. the 2f chat endpoints `/api/public/chat/optin` + `/api/public/chat/request` (carried forward from 2f-F2). These (provider-swap + net-new inbound + Turnstile/rate-limit) are the ONLY changes permitted to touch the frozen backend. CORS is browser-only — without 1f, a direct/non-browser POST with this client's slug or allowed-origin can spam-insert. NO client launches pre-1f.
- [ ] **Drip runner SCHEDULED via pg_cron — installed LAST in 1f (after the TextGrid swap + net-new inbound are validated on MANUAL ticks):** `cron.schedule` → `net.http_post` to the canonical PRODUCTION URL `https://cloud-spark-setup.lovable.app/api/public/cron/sequences` with the `x-cron-secret` header — NEVER a preview/`*-preview--*` alias. Pre-1f `cron.job` is empty BY DESIGN (nothing auto-fires real SMS during the build; all ticks manual). Run the **`?ping=1` confirm-promoted gate** (POST — there is no GET handler) to confirm the live bundle is the validated one BEFORE scheduling. (Spec §12 INFRA; build-log `1f-prep-deploy-lag-rootcause.md`.)
...
- [ ] Provider (TextGrid) number provisioned + forwarded + on site/GBP; per-client A2P Brand+Campaign approved (vets independently per client).
