...
- [ ] **[STAGE 3.5 §A — SECURITY] `clients_public` projection (closes `clients_anon_select_sensitive_fields`):** REVOKE anon's table-wide SELECT on `clients`; GRANT SELECT to anon on ONLY the 14 presentational columns (`slug, business_name, tagline, phone_display, address, hours, license_number, logo, brand_color, service_area, social_links, template_vars, review_link, quote_form_link`); KEEP the status-filtered anon row policy (`status='active' AND deleted_at IS NULL`); expose a **`security_invoker`** `clients_public` view over those columns; repoint the marketing data-loader at the view. Exclude ALL `twilio_*` + `call_forwarding_number` + `email` + `allowed_origins`; site renders from `phone_display`. *(A `security_invoker` view is required — the linter ERRORs on SECURITY DEFINER views — and needs anon to retain column-scoped base access, so full revoke/view-only isn't possible.)*
