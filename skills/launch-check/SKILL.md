...
- [ ] **[STAGE 3.5 §A — SECURITY] `clients_public` view (closes `clients_anon_select_sensitive_fields`):** DROP the broad row-level `clients_anon_select`; create a `clients_public` VIEW exposing ONLY presentational columns (`slug, business_name, tagline, phone_display, address, hours, license_number, logo, brand_color, service_area, social_links, template_vars, review_link, quote_form_link`; filtered `status='active' AND deleted_at IS NULL`); `GRANT SELECT TO anon`; repoint the marketing data-loader at the view. Exclude ALL `twilio_*` + `call_forwarding_number` + `email` + `allowed_origins`. Site renders the number from `phone_display` (NOT `twilio_number`).
