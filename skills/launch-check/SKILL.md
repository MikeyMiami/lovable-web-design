...
- [x] **[STAGE 3.5 §A — SECURITY — DONE 2026-06-14] `clients_public` projection (closed `clients_anon_select_sensitive_fields`):** REVOKE anon's table-wide SELECT on `clients`; GRANT SELECT to anon on the 13 presentational columns (`slug, business_name, tagline, phone_display, address, hours, license_number, logo_url, brand_color, service_area, social_links, template_vars, review_link`) + `status` + `deleted_at` (for the `security_invoker` view's WHERE filter); KEEP the status-filtered anon row policy (`status='active' AND deleted_at IS NULL`); `security_invoker` `clients_public` view over those columns; data-loader points at the view (Project 2). `quote_form_link` lives in the granted `template_vars` jsonb (not a top-level column). Exclude ALL `twilio_*` + `call_forwarding_number` + `email` + `allowed_origins` (live-verified 42501 denied). Verified: anon→`clients_public` returns data; audit 0. *(security_invoker required — linter ERRORs on SECURITY DEFINER; full revoke/view-only impossible.)*
...
- [ ] **Runner renders real SMS bodies from `templates` (3f finding — PRE-FREEZE MUST-FIX):** the runner SMS branch resolves `step.templateKey` → shared `resolveTemplate(clientId,key)` (client-override→global, the SAME resolver as the 2c dispatcher) → `dripMergeVars` incl. the per-contact tracked `review_link` — NOT `step.body ?? "[stub]…"` (which emitted literal `[stub] <key>` bodies for ALL drip SMS). **Confirm the SMS body templates are SEEDED as global rows for every step `templateKey`** (may never have been, since the runner never read them). Regression walk: an override for client A → A's drip SMS body = override copy, B's = global copy, neither `[stub]`. (Latent since 2e; would transmit `[stub]…` to real customers at 1f if unfixed.)
