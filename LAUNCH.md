...
**Stage 0 — Wiring** (setup, not skill-feeds): stable backend domain; enable pg_cron + pg_net; create the two storage buckets; set runtime secrets (CRON_SECRET, parent messaging-provider (TextGrid) auth token, Turnstile secret). Resolve the two ❓ confirm-in-Lovable items (email from-domain/ESP; native-AI invocation) before those specific build steps.
