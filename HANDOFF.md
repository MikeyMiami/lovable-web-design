...
- **NEXT: 1f STEP 2 = the NET-NEW inbound webhook layer** (`X-TextGrid-Signature` HMAC-SHA1 verify BEFORE any DB write; inbound SMS→CRM; real-time reply-driven drip exits; missed-call textback inferred from the voice status-callback `no-answer`/`busy`/`failed`; STOP/HELP/"pass" opt-out capture). Per-number webhook wiring at provision time. Not started. *(Doc inputs from the audit: `textgrid-provider` §3; inbound payload shapes + HMAC construction in the TextGrid Breeze/Voice docs.)* Subsequent 1f: Turnstile/rate-limit, A2P registration (step 6 — full field list in `docs/1f-step6-a2p-registration-field-requirements.md`), call-forwarding, the reactivation number pool, and **pg_cron scheduling LAST**.
