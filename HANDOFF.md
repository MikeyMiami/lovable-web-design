...
- **NEXT: 1f STEP 3 = LIVE Turnstile + rate-limiting** on all public lead-intake routes (incl. the 2f chat endpoints) — the §A launch-gate; provider-independent, so STUB/buildable now regardless of the auth confirm. **Remaining 1f after that:** A2P registration flow (full field list in `docs/1f-step6-a2p-registration-field-requirements.md`; also wires the per-number webhooks step 2 deferred), call-forwarding, the reactivation number pool, and **pg_cron scheduling LAST**. *(Ordering is Mikey's call.)* **LIVE flip remains gated on** the TextGrid parent-on-subaccount auth confirm (`textgrid-provider` §2/§7) + the form-encoding/content-type watch.
