...
> **🛑 SUPERSEDED — 2026-07-21: THIS POOL BUILD WAS REMOVED (it is NOT the go-forward path).** Reactivation now sends from the **client's OWN Telnyx number** (`clients.telnyx_number`, `provider='telnyx'`) via the per-client `reactivation` drip delivered by the **NORMAL send runner** (normal throttles + the 50/day · 2-per-20-min enrollment caps + dedup + a **REQUIRED consent attestation**). DELETED: `src/lib/reactivation/{runner.server.ts, pool.functions.ts, release.server.ts}`, `src/routes/api/public/cron/reactivation.ts`, `src/routes/_authenticated/admin.reactivation-pool.tsx`, the "Number Pool" admin nav link, and the `reactivation_numbers` / `reactivation_campaigns` / `reactivation_enrollments` tables + the `create_reactivation_campaign` / `claim_due_reactivation` RPCs. The global `reactivation` SEQUENCE remains (the per-client path uses it). **Retained below as the historical build record + reputational-isolation rationale**, which informs the deliberately-not-built future `telnyx_reactivation_number` seam. Rationale doc: `docs/reactivation-number-pool-spec.md`.
...
- **Assign (atomic + race-safe + cycle-safe) — ONE `SECURITY DEFINER` RPC.** The `reactivation_numbers.current_campaign_id ↔ reactivation_campaigns.pool_number_id` cycle means claim + campaign-create + back-link MUST be transactional (a crash between them must NOT strand a number `in_use` pointing at a campaign that was never created — and the number id isn't known-good until the claim, the campaign id isn't known until insert). App parses the CSV, then calls `create_reactivation_campaign(_number_id, _client_id, _contacts jsonb)` which in ONE transaction: (1) `UPDATE reactivation_numbers SET status='in_use' WHERE id=_number_id AND status='available'` → 0 rows ⇒ `RAISE EXCEPTION 'number_unavailable'` (rolls back); (2) validate `_client_id` is an active, non-deleted client (else RAISE); (3) `INSERT reactivation_campaigns(client_id, pool_number_id, status='dropping', total_contacts) RETURNING id`; (4) `UPDATE reactivation_numbers SET current_campaign_id = <new campaign id>` (back-link to the REAL id); (5) upsert `contacts` (`source='import'`) + `INSERT reactivation_enrollments` (step 0, `next_run_at=now()`); (6) campaign → `dripping`. **All-or-nothing** — a mid-flow crash leaves neither a stranded `in_use` number nor a dangling link. (`SECURITY DEFINER`, `search_path=public`, service_role-only — same atomic-multi-step pattern as `claim_due_*` / `export_client_bundle`.)
...
- `runReactivationTick()`: claim due `reactivation_enrollments` (`status='active', next_run_at<=now`, `FOR UPDATE SKIP LOCKED`, bounded; fairness across campaigns). For each: load campaign → pool number + client; resolve `to`=contact `phone_e164`; **SMS-window gate** (defer if outside 9–7 client tz); render `resolveTemplate(client_id, step.templateKey)` + `dripMergeVars`; `sendSmsWithRetry({to, from: poolNumber, body, sendingAccountSid: cfg.masterAccountSid, auth: masterAuth, mode: cfg.mode})` — where **`masterAuth` is built EXACTLY as the frozen runner does**: `` `${cfg.masterAccountSid ?? ""}:${cfg.masterAuthToken ?? ""}` `` (`runner.server.ts:274`, also `reply.functions.ts:71`); **reuse that construction, do not reinvent it**; materialize via `insertOutboundMessageAdmin`; advance/reschedule (mirror the frozen advance). Then the **release-check** per touched campaign. Emit `events` (`reactivation_sent`, `reactivation_campaign_completed`).
