...
- **REMIX (client site)** — a copy of a template with **TWO per-client `.env` values, set at different times**: `VITE_CLIENT_SLUG` at remix time (blank = demo mode), and `VITE_SITE_URL` at DOMAIN DAY only (the client's real `https://domain.com`; empty until then, which deliberately keeps the site `noindex` + `Disallow: /`). Both are pre-declared in the template `.env`. **`VITE_PLATFORM_API_HOST` must ALWAYS stay `https://app.pierceworks.co`** — it is the BACKEND host, never the client's domain; overwriting it silently kills every form POST while the site looks perfect. The shared-backend URL + anon key are HARDCODED constants in the template code (Lovable's remix strips `VITE_SUPABASE_*` from `.env`, so they must NOT depend on `.env`). Frontend-only forever.
...
- **VERIFY AGAINST REALITY, NOT DOCUMENTATION.** Docs and skills drift from what actually shipped, and the drift is invisible until it costs you. Confirm code against the repo (read the file / diff the commit), and confirm DATA against the database (query it). A change report describing intent is not evidence the change landed. Real examples: a One-Year handoff documented for months that had zero code behind it; three "known bugs" already fixed while the docs still listed them; a notification body in the docs that no client had ever received.
- **DATA is not in migrations.** Message + notification copy lives in the `templates` table, drip timing in `sequences`, per-client config in `clients`/`template_vars` — none of it is seeded by migration (that would violate the schema-only rule above). So for anything copy- or timing-related, **the database is the only source of truth** and a doc quoting a message body is a secondary record that may be stale.
- **When editing stored copy, prefer surgical SQL over rewriting a body.** SQL editors collapse newlines on display, so a body pasted back into an `UPDATE` silently loses its formatting. Use `body || E'\n\n…'` to append (with a `NOT LIKE` guard for idempotence) and `replace(body, $x$old$x$, $x$new$x$)` for targeted edits — dollar-quoting avoids apostrophe-escaping bugs in copy full of contractions.
