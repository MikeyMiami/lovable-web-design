...
## SEO = DATA
`content_pages` rows; templates render them; publishing never touches Lovable projects. `/admin/seo`: AI-seed (`proposeSeoMap`, never writes) -> correct vs the real GBP -> Save (`saveSeoMap`, server merge) -> Seed Core-30 -> AI-write -> Publish. Geo/supporting monthly.

## Client PWA - Inbox
- **Archive**: nullable `conversations.archived_at`; writes via role-verified `setConversationArchived` (scoped by BOTH id and client_id). Swipe on touch, hover overlay at `md`+. **Two invariants:** `admin.conversations.tsx` stays UNTOUCHED (agency keeps full visibility); a new inbound AUTO-UNARCHIVES via DB trigger `trg_unarchive_on_inbound` - MUST be a trigger, since inbound arrives via Edge Functions and would bypass app code, burying a lead.
