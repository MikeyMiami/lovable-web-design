...
## Messaging / providers
`clients.provider`: **telnyx = DEFAULT** - single account, ONE `TELNYX_API_KEY` + ONE `TELNYX_PUBLIC_KEY`, no per-client credentials. **Only THREE per-client columns: `telnyx_number`, `telnyx_brand_id`, `telnyx_campaign_id`** (+ `telnyx_a2p_status`). **The messaging profile and TeXML voice app are SHARED platform-wide** - one profile + one voice app, every client number on both (numbers under different 10DLC brands/campaigns may share a profile; 10DLC doesn't govern voice). `telnyx_messaging_profile_id`/`telnyx_texml_app_id` are unused columns, removed from Settings/onboarding/checklist - never read at runtime (sends use `from`; inbound resolves by `to`). **Number Pooling must stay OFF** - it overrides the `from`, so Client A would send from Client B's number and the reply would land in the WRONG inbox. Webhooks = `telnyx-*` Edge Functions, Ed25519. **Inbound voice IS signed; status/AMD callbacks are NOT - hard-verify inbound only, or you kill AMD and missed-call textback.** **textgrid = FROZEN LEGACY** (existing clients only). Never send an SMS body with residual `{tokens}`.
...
**pg_cron LIVE since 2026-07-29** - job `drip-runner`, `*/2 * * * *` -> **`https://app.pierceworks.co/api/public/cron/sequences`**, `timeout_milliseconds := 30000`. **Always the app host, never lovable.app** - that 302s and `net.http_post` does NOT follow redirects, so it ticks forever with zero sends and zero errors. Verify BOTH `cron.job_run_details` AND `net._http_response` (the post is async). `?ping=1` is POST-only.

**Demo clients** (`is_demo`, `demo_expires_at`; agency `/agency/demos`): sales props - `status='active'` so the template can read them, but no messaging config, refused by the write resolver (both resolve fns null on `is_demo`), filtered from THREE reads only (admin switcher, Payment Access, `listSeoStatus`). Slug `demo-...`, 14d expiry, purge `is_demo`-guarded. Services -> `services_structured`; photos -> `site_assets.work_examples`.
...
**TWO paths into One-Year:** clickers enrol from the FUNNEL (`r/$token.ts`, `r/rate.ts`); contacts who ignore all 4 texts enrol via `handoffToOneYear()` in the runner's terminal branch. Both skip opt-outs.
...
Opt-out = TWO LAYERS. App (`parseIntent`): STOP synonyms + sole-word "pass" -> opt out + exit ALL active drips; START/YES/UNSTOP -> re-opt-in; HELP is NOT app-handled. Telnyx: auto opt-out is DEFAULT and cannot be disabled - auto-confirms STOP, hard-blocks later sends (40300, caught by the runner's D13 sync). **Telnyx blocks are PROFILE-WIDE: STOP to any client's number blocks that phone for EVERY client.**
...
## SEO = DATA
`content_pages` rows; templates render them; publishing never touches Lovable projects. `/admin/seo`: AI-seed (`proposeSeoMap`, never writes) -> correct vs real GBP -> Save -> Seed Core-30 -> AI-write -> Publish.

## Client PWA - Inbox
- **Archive**: nullable `conversations.archived_at` via role-verified `setConversationArchived` (scoped by BOTH id and client_id). **Two invariants:** `admin.conversations.tsx` stays UNTOUCHED (agency keeps full visibility); a new inbound AUTO-UNARCHIVES via DB trigger `trg_unarchive_on_inbound` - MUST be a trigger, since inbound arrives via Edge Functions and would bypass app code, burying a lead.
- **Lead-type avatar**: the row circle shows lead TYPE. Match order (engagement outranks acquisition): Star (review/reactivation, `contacts.status`) -> Percent (discount, `enrollments.sequence_key`) -> PhoneMissed -> MessageSquare (`web_form`|`chat_widget`) -> User. **The discount form writes `source='web_form'` like the lead form**, so only the enrolment join identifies a discount claimer.
