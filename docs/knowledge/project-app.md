...
Opt-out at the inbound webhook (`_shared/core.ts parseIntent`): STOP synonyms (STOP/STOPALL/UNSUBSCRIBE/CANCEL/QUIT/END) + sole-word "pass" -> opt out + exit ALL active drips; START/YES/UNSTOP -> re-opt-in. **HELP is NOT app-handled** (routes as a plain reply) - HELP auto-response must be verified/enabled at the Telnyx profile level before a real client launches.
