...
- **F.1 Gate:** 1f shipped; cron scheduled at prod URL; client row + config present; **all 9 required `template_vars` populated** (the RED list == `src/lib/admin/required-template-vars.ts`: `company_owner_first_name`, `company_name`, `review_request_link`, `discount__on_referral`, `discount_amount`, `quote_form_link`, `about_us`, `services`, `differentiators`; `company_website_link` = YELLOW, set at D.5; `website_terms_page_link` no longer required 2026-07-23); timezone/hours/window/caps set; review link+Place ID+threshold; number provisioned+forwarded+placed; **A2P Brand+Campaign approved**; `allowed_origins` set; site remixed+pointed at shared env; native bot-shield live on all forms (`POW_SECRET` set — Turnstile RETIRED 2026-07-22).
