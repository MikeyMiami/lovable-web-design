...
| P2 | **Stage 1f shipped — HARD GATE ("NO client launches pre-1f").** TextGrid send swap + inbound/voice webhooks (`X-TextGrid-Signature` HMAC-SHA1) + live bot-shield (invisible native PoW — Turnstile RETIRED 2026-07-22) + rate-limit on all 3 public forms + pg_cron drip-runner scheduled LAST at the prod cron URL (`app.pierceworks.co`, **not** the lovable.app host). | inbound live-validated 2026-07-11; **cron scheduled + verified 2026-07-29**; verify the full 1f flip before a paying client |
...
3. **Phase A.5 — Review + provision.** Onboarding queue → `/admin/review` → **Finalize & Activate** (`provisionClientOwner`: SILENT `createUser` + `client_owner` grant + flip `status`→`active`; **no email**). The client's welcome email is a separate step at the very end — **F.4**.
...
6. **Phase D — Remix the site.** Remix template (history=No) → rename → set the slug (`VITE_CLIENT_SLUG`; the Supabase URL/key are hardcoded in the template code, NOT the remix `.env` which Lovable strips) → publish → connect domain → add domain to `allowed_origins`. (Turnstile-hostname step RETIRED 2026-07-22 — native bot-shield needs no hostname registry.)
...
9. **SEO program** runs continuously post-launch for EVERY client (see §5) — single tier since 2026-07-29, no Pro gate.
...
### DOMAIN MAP [read this before touching any URL] — 4 domains, 4 jobs
| Domain | Job | Seen by |
|---|---|---|
| **`<clientdomain>.com`** | the client's marketing site (the Remix). Also serves `/review/<token>` → 302 → the funnel. | their customers |
| **`app.pierceworks.co`** | THE APP — admin view, client PWA, `/login`, `/set-password`, `/download`. All app links build from `PUBLIC_APP_ORIGIN ?? https://app.pierceworks.co`. | agency + clients |
| **`notif.pierceworks.co`** | Resend sender domain — ONE verified domain, per-purpose addresses: `alerts@` = owner notifications; `onboarding@` = client welcome + onboarding-link emails (via `RESEND_WELCOME_FROM`/`RESEND_ONBOARDING_FROM`, reply-to → monitored inbox). | email headers |
| **`reviewbatch.com`** | the shared REVIEW-FUNNEL backend. Serves **only** `/api/public/r/*`; every other path 302s to the app (`server.ts hostRedirect`). Hidden behind client domains; used directly in texts only when `review_link_domain` is blank. | review-page visitors |
> ⚠️ **The trap:** env `PUBLIC_APP_URL` / `VITE_PUBLIC_APP_URL` is the **REVIEW-link base** (deployed = `reviewbatch.com`), NOT the app origin — its only consumer is `trackedLinkUrl()` in the cron runner. Building app URLs from it produces `reviewbatch.com/set-password`, which Supabase's redirect allowlist rejects (real bug, 2026-07-23). Rename to `REVIEW_LINK_BASE` is backlogged.
> **Why a separate review domain at all:** the funnel writes to the DB, so it lives once in the shared backend and needs one address — and that address must never be the app domain, because review links ride inside bulk SMS where a spam flag would poison the domain your logins and auth emails depend on.
...
- **A.5.3 Finalize & Activate [SPLIT from the client email 2026-07-23]** — `provisionClientOwner`: (1) **silently** create the login (`auth.admin.createUser`, `email_confirm:true`) for `notification_email ?? email` — **NO EMAIL IS SENT**, (2) audited `client_owner` grant, (3) **flip `status` pending→active** (now anon-visible + automation-eligible — required before the remix will render real data), (4) surfaces the Remix handoff checklist (slug / `VITE_CLIENT_SLUG` / `allowed_origins`; the site_style row was removed 2026-07-22). *The client hears nothing at this point by design — at finalize their site doesn't exist yet, so a "view your website" link would be dead. Their one email is **F.4**, at launch. Supabase now sends zero emails in this design; custom SMTP is backlog, not a prereq. Auth **Site URL** is set to `https://app.pierceworks.co` (done 2026-07-23) — still correct to keep, it's the fallback landing for any future auth email.*
- **A.5.alt DECLINE [2026-07-22]** — the other exit from the queue: "Decline" on the `/agency/onboarding` row or "Decline & delete" on `/admin/review` = **PERMANENT FULL PURGE** (confirm-guarded; `declineOnboardingClient` fn refuses any non-`pending` client): storage prefixes wiped (submission JSON + all uploads incl. token draft uploads) + hard row delete (FK cascades clear all data tables; the claimed onboarding-token record survives with `created_client_id` nulled). Not recoverable. Button hard-requires an email + `window.confirm` (sends a real email).
...
- **C.3 Campaign** — qualify use-case → `POST /campaigns/campaign` with `usecase`, `description` (must contain "message frequency may vary"), opt-in/out/help, `sample1..5` (real drip copy, real names, ≥2 with STOP), `termsAndConditionsLink`+`privacyPolicyLink` (REQUIRED), `embeddedLink:true` → `campaignId`. **Assemble the pack BY HAND from `docs/a2p-compliance-copy-source-of-truth.md` (§G Brand fields, §F per-client campaign templates, §E the approved Review Harvest reference, §H decline triggers). There is NO admin A2P tool and none is planned — registration is a MANUAL out-of-app process by operator decision 2026-07-29, revisit only at scale.** All copy/URLs come from `/a2p-site-compliance` — do NOT re-author.** DCA2 $15/submission; `expediteCampaign:true` +$27.
- **C.4 Buy + attach number** — local area code matching the market → attach to campaign. **SHARED PLATFORM OBJECTS [2026-07-29 — attach every client's number to BOTH, same values every time]:**
  - **Messaging profile — "Pierceworks"** · `40019f80-6b8f-473f-afdd-23a7f32151c5`
  - **Voice application — "Pierceworks Voice"** · `3008335895855105762`
  **Confirmed supported by Telnyx support:** numbers under DIFFERENT 10DLC brands/campaigns can sit on the SAME messaging profile (each individual number maps to exactly one campaign/brand), and one voice application can serve numbers regardless of their campaign — 10DLC governs messaging only and does not touch voice. **Only `telnyx_number`, `telnyx_brand_id` and `telnyx_campaign_id` are per client now** — the profile and voice-app IDs were removed from Settings/onboarding/checklist (they were never read at runtime: sends use the `from` number, inbound resolves the client by the `to` number). **⚠ NEVER enable Number Pooling on the shared profile** — it overrides the `from` and would send Client A's message from Client B's number, so the customer's reply would resolve to the WRONG client's inbox (`telnyx-sms-inbound` matches on the `to` number). It is opt-in and off by default; leave it off. Also note the profile's **daily spend limit is shared across all clients** — leave unset or size it for aggregate volume. Scale check: no per-profile or per-app throughput ceiling exists — SMS throughput is per number (~6/min long code) and per brand/campaign, and voice concurrency is account-wide *channels*, so sharing costs no capacity.
...
- **C.6 Write + place the number [TWO admin inputs — exact locations; do AFTER the number is provisioned; can be before OR after the site remix (the site reads data LIVE, so a late fill updates it on next page load); A2P approval gates SENDS, not these inputs]:**
  ...
  2. **Website display phone** — Admin → Settings → **"Business Identity" card → "Phone (display)"** input. *The phone number the client's WEBSITE shows — the **Call-Now pill in the site header on every page** (added 2026-07-27), the footer, and every tap-to-call. Set it to the SAME Telnyx number so customer calls/texts flow through the platform; the live site updates on next page load.* **⚠ `phone_display` only re-normalizes ON SAVE** — paste the digits and SAVE the card, or the pill renders raw digits like `4197500242` instead of `(419) 750-0242` (the site formats it via `formatPhoneUS`, but the stored value is what the drips and the welcome email merge). The pill's `tel:` href comes from `phone_e164`, so both fields must be right.
...
- **D.1** Remix the matching `Template — {Style}` project (exact copy, no AI/credits). **D.2** Rename. **D.3** Set the ONE per-client value — the **slug** (`VITE_CLIENT_SLUG=<slug>` in `.env`, or a `client-slug.ts` constant). ⚠ **The Supabase URL + anon key are NOT in the remix `.env`** — Lovable's remix strips `VITE_SUPABASE_*`; they live as HARDCODED constants in the template's `client-data.tsx` (correct by design — same backend forever). If the published site flashes the client then reverts to demo after ~2s, the connection constants are missing from the template code, not the `.env`. Also run Lovable's dependency-audit **Fix** (seroval) if publish is blocked and the template's `package.json` override wasn't inherited. **D.4** Connect the client's domain (marketing at root, app at `app.theirdomain.com`). **D.5 — the "DOMAIN IS LIVE" TRIO [MUST-DO, in this order — the same moment, three Settings fields; complete scope verified 2026-07-22, nothing else needs the domain]:**
  ...
  4. **⛔ `VITE_SITE_URL` on the REMIX `.env` → then REPUBLISH** = `https://theirdomain.com` (no trailing slash). **✅ PRE-DECLARED SINCE 2026-07-29 — the field is already in the remix's `.env`, empty. Just fill it in.** The template `.env` now ships `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` / `VITE_PLATFORM_API_HOST` / `VITE_CLIENT_SLUG=` / `VITE_SITE_URL=""`, and a remix inherits all of it EXCEPT the two `VITE_SUPABASE_*` lines, which Lovable strips on remix — harmless, they're hardcoded as fallbacks in `client-data.tsx`. (Before this fix no field existed at all, because the code reads `VITE_SITE_URL` with a fallback so nothing forced it to be declared — which is exactly how it got confused with the API host on 2026-07-27.) **DO NOT repurpose `VITE_PLATFORM_API_HOST` for the client's domain** — that is the BACKEND host and must stay `https://app.pierceworks.co`; overwriting it points every lead/discount/chat POST at the marketing domain and silently kills form capture (observed 2026-07-27 during the domain test). One is where the site *talks to*; the other is what the site *calls itself*. **ADDED 2026-07-26 — this is now a HARD GATE, not a nicety.** The template resolves absolute URLs at runtime and treats an origin as indexable ONLY when `VITE_SITE_URL` is set and the host isn't `*.lovable.app`. Until it is set **and republished** (env bakes at PUBLISH, not at save), the client's real live domain serves `<meta name="robots" content="noindex,follow">` on every page and `/robots.txt` = `Disallow: /`, with canonical/og:url still pointing at the TEMPLATE's domain — a silent, total SEO blackout with no warning anywhere in the app. Conversely this is deliberate protection: every pre-launch preview is unindexable by construction. **VERIFY FROM OUTSIDE after republishing** (curl, not the browser): `<meta name="robots">` = `index,follow`, `/robots.txt` = `Allow: /` with an absolute `Sitemap:` line on the new domain, `/sitemap.xml` `<loc>`s on the new domain, and `<link rel="canonical">` = the client's domain. See [[project_backlog_pre_launch_validation]] 2c.
  *(NOTHING of the platform's own subdomains — app./notif.pierceworks.co — ever goes in any of these; the platform talks to itself same-origin. Future candidate build: ONE "Domain" input that derives all four.)*
...
- **E.3.5 SITE COPY pass [BUILT 2026-07-25 — Admin → "Site Copy" tab; DATA, live on next page load, no republish]:** any time after Finalize (recommended: right after the remix first renders). The site already shows literate MERGED DEFAULT copy for every section (built from the client's values — zero effort baseline). This pass upgrades it: **Generate & apply all with AI** (global tone/length; ~10 sequential generations) → then READ each section and edit/regenerate any that miss → Save persists per-client overrides (`template_vars.content`). **Ownership boundary [LOCKED]:** Site Copy = **TWELVE slots [updated 2026-08-02]** — the 8 prose slots (`home.intro`, services intros, About Overview/Approach, Contact/Gallery/Discount intros), **two conversion MICRO-COPY slots** (`home.call_bar_label`, the click-to-call pill in the header, default "Call Now"; `home.cta_button`, the hero button that opens the chat popup, default "Chat with us now"), **and two HERO slots** (`home.hero_headline`, the large display line — blank shows the SEO H1 there instead; `home.hero_subhead`, the supporting line between headline and button — blank renders no line). The two button labels are 2–5 word BUTTON LABELS, not sentences — the batch generator includes them, so after a "generate all" run eyeball both and revert to the defaults if the AI wrote a sentence. `home.cta_button` only appears on-site when the client's chat widget is enabled. The homepage H1/title/SEO body + ALL service/category/geo/supporting pages = the SEO tab (E.4) — the three overlapping cards carry "SEO note" labels in the UI. The 3 homepage/services slots generate in plain search language (category + city) by design; blank a slot + Save = revert to the merged default.
  - **⚠️ `home.hero_sub` was RENAMED to `home.intro` [2026-08-02].** It never rendered under the hero — it renders in the intro section BELOW it, and the old label ("Homepage — Hero subtext") sent operators to the wrong field. A `jsonb_set` migration carried existing values across and both templates dual-read (`home.intro` then `home.hero_sub`), so the rename is safe in any deploy order.
  - **⚠️ HERO/H1 SPLIT [2026-08-02].** The hero H1 is no longer the big display line. The ONE `<h1>` still carries the SEO keyword phrase (`page.h1` → `template_vars.hero_headline` → `business_name`) but renders as a small kicker ABOVE the display line; the large serif line is now the `home.hero_headline` slot. The SEO tab still owns the H1 text; Site Copy owns what the visitor actually reads first. Same pass: an operator's `home.intro` now BEATS the SEO body's intro paragraph — previously, publishing the Core-30 home page silently killed that copy while admin still showed it as saved.
  ...
  - **POST-LAUNCH (yellow-checklist item — after the site is live on its domain):** AI-write the pages → **Publish** → review the RENDERED pages on the live site (they only become viewable/checkable once the site is up — which is why content finalization waits). Ongoing geo/supporting pages ~monthly (every client — single tier since 2026-07-29, no `seo_cadence` gate).

### Phase F — Verify + go live (`/launch-check` §E — data QA)
- **F.1 Gate:** 1f shipped; cron scheduled at prod URL; client row + config present; **all 8 required `template_vars` populated** (the RED list == `src/lib/admin/required-template-vars.ts`: `company_owner_first_name`, `company_name`, `review_request_link`, `discount__on_referral`, `discount_amount`, `about_us`, `services`, `differentiators`; `company_website_link` = YELLOW, set at D.5; `quote_form_link` = fallback-only off `company_website_link`; `website_terms_page_link` no longer required — all three removed from the required list 2026-07-23); timezone/hours/window/caps set; review link+Place ID+threshold; number provisioned+forwarded+placed; **A2P Brand+Campaign approved**; `allowed_origins` set; site remixed+pointed at shared env; native bot-shield live on all forms (`POW_SECRET` set — Turnstile RETIRED 2026-07-22).
...
- **F.3 Live.**
- **F.4 SEND CLIENT WELCOME [BUILT 2026-07-23 — the LAST step; the client's ONE and ONLY email]** — Admin → `/admin/review` → **"Send client welcome"** card (renders only when `status='active'`). Edit the intro message in the textarea if you want (**Save message** persists it per client; the layout preview shows where the links land), then send. The email carries 3 links: **their website** (`template_vars.company_website_link`) · **set a password** (`app.pierceworks.co/set-password`, single-use) · **download the app** (`/download?c=…`). *The fn hard-refuses unless the client is active AND `company_website_link` is set — that's the code-level guard enforcing "site must be live before the client is contacted."* The client then: views the site → sets a password → installs the branded PWA. **Re-send is allowed** and mints a fresh link (invalidating the old one) — this is also the **agency-side password reset** until self-serve reset is built. Sender = `RESEND_WELCOME_FROM ?? RESEND_FROM` (currently `Pierceworks <onboarding@notif.pierceworks.co>`; reply-to `RESEND_WELCOME_REPLY_TO` → monitored inbox, since the sending address can't receive); delivery failures THROW (no silent fail). Footer stamps send date+time so repeat sends never fold in Gmail.
...
**Tier gating: NONE — single tier since 2026-07-29.** Every client gets the full Core-30 structure, the map editor, single-pass ai-write, **the 8-pass deep-write, the monthly cadence board and rank-map-driven expansion.** The `seo_cadence` entitlement and `src/lib/entitlements.ts` are deleted; the agency monthly-review worklist covers ALL active clients.
...
8. **Monthly cadence board `/agency/seo-status`** (ALL active clients — the Pro-only filter was removed 2026-07-29) — last-reviewed / next-due chips, due badge; per-client "SEO Updated" button (resets 30-day clock) + `topical_status` (building|established|dominating) + `close_towns[]`.
...
| 9 | **Chat Widget (capture-first, no AI)** | Corner bubble → greeting → lead form (First/Last/Phone+msg, consent, invisible PoW shield) → `/api/public/chat/optin`. Source `chat_widget` | enrolls the Lead-Form drip immediately (BH branch); message → Conversations tab (`channel='chat'`) | "New Website Chat Widget Lead" |
...
1. **✅ RESOLVED — verified against the live DB 2026-07-29.** The `missed_call_internal_notify` template body is exactly *"You missed a call from {caller_phone} at {call_time}, so we sent them a text."* — no bracketed placeholder. Items **#2** (`{first_name}` → `{full_name}` in the review terminal notif) and **#4** (the "(Do NOT reply to this message…)" footer) are ALSO already resolved in the live templates; only this doc and `mobile-app` still showed the old copy, now corrected. **Still open: #3** (`{your_message}` vs `notes` — `/api/public/intake` stores the lead message as `notes`; unverified whether the var mapping bridges it) and **#5/#6** (One-Year grammar, `{discount__on_referral}` double-underscore). Original writeup: **🚩🚩 Missed-call notif (#5): literal `[Open conversation button]`** sits in the message BODY. Should be an `action`-jsonb button (like the day-10 reminder), not bracketed text. **Highest priority.**
...
Still external / open:
- **✅ CRON DRIP RUNNER IS LIVE (scheduled 2026-07-29).** Job `drip-runner`, `*/2 * * * *`, `net.http_post` → `https://app.pierceworks.co/api/public/cron/sequences` with `x-cron-secret` and `timeout_milliseconds := 30000`. (Was OFF from 2026-07-14 to 2026-07-29 — no drip sends processed for ANY client during that window.) **⚠ The host must be the app custom domain, NOT `cloud-spark-setup.lovable.app`** — that 302s and `net.http_post` does not follow redirects, so the job would tick forever with zero sends and zero errors. **Verify BOTH `cron.job_run_details` AND `net._http_response`** — the post is async, so pg_cron reports success merely for queueing.
