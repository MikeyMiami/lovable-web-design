...
- **D.1** Remix the matching `Template — {Style}` project (exact copy, no AI/credits). **D.2** Rename. **D.3** Edit `.env` → change ONE line `VITE_CLIENT_SLUG=<slug>` (Supabase URL+anon key already correct — always Project 1). **D.4** Connect the client's domain (marketing at root, app at `app.theirdomain.com`). **D.5** Add domain to `allowed_origins`. (The old "+ Turnstile hostname" step is RETIRED 2026-07-22 — the forms run the native PoW bot-shield, no hostname registry.) **D.6** Set **Review link domain** (admin → Settings → Review Config) to `https://theirdomain.com` so review SMS links carry the client's own domain (`/review/<token>` 302 → backend funnel; route is baked into the template). Verify: `theirdomain.com/review/test` redirects to the funnel. NEVER set this before the domain is live — blank = shared funnel domain (always works).
