...
- **D.1** Remix the matching `Template — {Style}` project (exact copy, no AI/credits). **D.2** Rename. **D.3** Edit `.env` → change ONE line `VITE_CLIENT_SLUG=<slug>` (Supabase URL+anon key already correct — always Project 1). **D.4** Connect the client's domain (marketing at root, app at `app.theirdomain.com`). **D.5** Add domain to `allowed_origins`. (The old "+ Turnstile hostname" step is RETIRED 2026-07-22 — the forms run the native PoW bot-shield, no hostname registry.) **D.6 — "DOMAIN IS LIVE" field set [MUST-DO, complete scope verified 2026-07-22 — these CANNOT be finalized earlier]:**
  1. **Review link domain** (Settings → Review Config) = `https://theirdomain.com` → review SMS links become `theirdomain.com/review/<token>` (302 → backend funnel; route baked into the template). Verify `theirdomain.com/review/test` redirects. NEVER set before the domain is live — blank = shared funnel domain (always works).
  2. **`template_vars.company_website_link`** = `https://theirdomain.com` (Template Variables editor; amber-flagged when missing) — merged in drips AND is the runner's fallback for `{quote_form_link}` in Missed-Call Textback SMS #1; a no-prior-domain client has it BLANK until now.
  ~~3. website_terms_page_link · 4. contact_email domain-match~~ **SUPERSEDED 2026-07-22 [operator]: A2P approval runs on a SEPARATE site, NOT the client's marketing site** — registration links/domain-match rules apply to that approval site; the client site's compliance surface still matters for the lead-form consent copy itself, just not for registration. (⚠️ Broader a2p-site-compliance doc reconciliation to the separate-approval-site model is PENDING — scope with the operator before the first real registration.)
  *(allowed_origins = D.5; site URL on the Google Business Profile = C.6 — already steps.)*
