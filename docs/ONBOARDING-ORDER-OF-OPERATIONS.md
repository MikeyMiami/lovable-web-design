...
- **A.5.3 Finalize & Invite** — `provisionClientOwner`: (1) magic-link invite to `notification_email ?? email`, (2) audited `client_owner` grant, (3) **flip `status` pending→active** (now anon-visible + automation-eligible), (4) surfaces the Remix handoff checklist (slug / `VITE_CLIENT_SLUG` / `allowed_origins`; the site_style row was removed 2026-07-22). *Invite-landing prereq [SETTLED 2026-07-23]: Supabase Auth → URL Configuration → **Site URL = `https://app.pierceworks.co`** (one-time backend dashboard field — it defaulted to the original `*.lovable.app` origin and connecting the custom domain does NOT auto-update it; the magic link lands wherever this points because `VITE_INVITE_REDIRECT_URL` stays unset; the Redirect-URLs `/**` wildcards already allow it). Still open: custom SMTP, or the invite may land in spam.*
