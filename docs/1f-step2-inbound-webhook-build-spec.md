...
- **`POST /api/public/sms/inbound`** — verify sig → resolve by `To` → find-or-create contact (`From`) → dedupe by `MessageSid` → `insertInboundMessageAdmin` + `inbound_sms` event → **keyword parse** (case-insensitive, trimmed): STOP/STOPALL/UNSUBSCRIBE/CANCEL/QUIT/END and START/YES/UNSTOP match as the **first word**; the app's **`pass`** matches as the **SOLE word only** (the whole message is just "pass" + optional punctuation — NOT `\bpass\b`-anywhere, which false-positives "pass this along / pass it to your manager"). STOP/`pass` → set `opted_out_at` + `exitActiveEnrollments`(all); START → clear `opted_out_at`. Else (a real reply) → **real-time exits**: one_year_followup active → `exitActiveEnrollments` + `writeNotificationByTemplateKey("one_year_reply_interest_internal")`. (missed-call sms2 reply-skip needs no special handling — the runner's `skipIfReplied` now works because the inbound message exists.) → respond `<Response></Response>`.
...
5. **Opt-out capture — ⭐ HIGHEST-STAKES (end-to-end TCPA goes live here); exercise hardest:**
   ...
   - (b) `Body="pass"` (SOLE word; "Pass."/"PASS!" ok) → same as STOP. **`Body="passport"` / `"my password"` / `"pass this along"` / `"pass it to your manager"` → does NOT opt out.** (Sole-word, not `\bpass\b`-anywhere.)
...
- **[RESOLVED — tightened post-d8df4dd] `pass` scope.** `d8df4dd` matched `pass` via `\bpass\b`-anywhere (would opt out "pass this along"). **Fix: `pass` opts out only as the SOLE word** of the message (same tokenization as the STOP first-word parse; opt out iff exactly one alpha token === "PASS"). STOP set stays first-word. Re-spec'd as a one-function change to `parseIntent` (STUB, `RUNNER_VERSION → v20260617-2`). Standard STOP set + TextGrid native suppression unchanged; the app still MUST set `opted_out_at` so OUR runner stops.
