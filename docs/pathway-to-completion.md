...
### Phase B-Inbox — Inbox/Conversations enhancements *(app-feature track; PLANNED — runs AFTER B-design closes, its own unit)*
- **[PLANNED — scoped 2026-06-20, parked.]** Full scope + build plan: **`docs/inbox-conversations-enhancements-scope.md`**. Mikey's confirmed 6-feature set (priority-ordered): (1) add name/email to a contact from inside the conversation, (2) filter recent/oldest/**unread**, (3) **delete or archive** a conversation, (4) archived-conversations view, (5) notes per conversation (linked to contact), (6) search by name/number.
