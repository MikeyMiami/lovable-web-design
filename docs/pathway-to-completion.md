...
- **[PLAN LOCKED 2026-06-30 — SEO system]** author **`seo-build`** (site/template structure + technical + GBP-consistency + the **Core 30** + dynamic content-render routes) **+** **`seo-content`** (content-production standard — 8-pass quality, local specificity, per-page-type patterns, the topical-vs-geographic decision + monthly loop) **BEFORE styles 2–6** so every style template bakes the SEO scaffolding in once. Then (later module) the **content store** (`content_pages` additive table — the CMS-backed endpoint) + the **content-automation tool** (Core-30-Agent equivalent) + rank-map input. Reconciles `website-structure` to the Core 30 (supersedes the generic service-area-lander model). Adopts the expert local-SEO system (GBP↔website mirror, entity-based, editorial internal linking). **Full analysis + signed-off master plan: `docs/seo-system-analysis-and-skill-plan.md`.** **[UPDATE 2026-07-03]** the SEO **content-store arc is BUILT + validated** (STORE-1 store → STORE-2 render routes → 3a map → 3b seed → 3c-1 manage → 3c-2 write; single-city topical Core-30, all `golden-master-v1.7`, `docs/build-log/stage-seo-store-*`). The remaining path to fully realize the method is the **4-slice SEO Completion Roadmap — `docs/seo-completion-roadmap.md` (SOURCE OF TRUTH for the SEO build sequence)**: Slice 1 content-quality (per-type §3 prompts + structure) → Slice 2 images v2 (`images jsonb`) → Slice 3 multi-location (LOC-1/2/3 + geo/supporting creation — the biggest gap) → Slice 4 ongoing content-automation tool (§4 8-pass + §5 research + §7-8 rank-map loop). Gap basis: `docs/seo-system-audit-2026-07-03.md`. Each slice updates its status in the roadmap on validation.
...
> **[DONE — B-design complete + validated 2026-06-20.]** App-layer build (no migration, no re-tag; baseline stays `golden-master-v1.7`), shipped in 3 validated slices: **Slice 1** = the 5 service-role ticket write-fns + `isAdmin`/`isMemberOf` helpers + notify/owner-email-stub (grant-layer write-lockout + harness battery green); **Slice 2** = client PWA (nav restructure Stats=Home / 3 tabs / ☰ menu, payment-gate intercept [TOTAL], read-only Account, Edit/Support `TicketSurface` + `ticket-upload.ts` + `open_ticket` Alerts); **Slice 3** = per-client admin Edit Requests + Support (`AdminTicketSurface` → `postAgencyReply`/`setTicketStatus`, Pending/All/History). Full client↔agency round-trip proven on both kinds + tenant isolation + payment gate. Specs: `docs/phase-b-design-core-build-spec.md` + `docs/phase-b-design-prompt{1,2,3}-lovable.md`. Skills mirrored: `scratch-foundation`/`mobile-app`/`admin-view`. **Cross-tenant agency-view = Phase C** (not built here). Config carried: custom SMTP (before real client invites).
...
- **`client-assets` bucket caps — prerequisite for the ticket-upload feature.** When `recordTicketAttachment` (the §4c upload fn) is built, also set the `client-assets` bucket's **file-size limit = 25 MB** and a **superset MIME allow-list** (`image/*`, `application/pdf`, `video/mp4`, `video/quicktime`, `video/webm` — must still cover existing logo/asset uploads). **Storage-dashboard config, not SQL/code**; deferred from the v1.7 batch (no §5 dependency). The precise per-file gate is the server fn + client-side; this bucket setting is the coarse backstop (Decision #5, three-layer enforcement).
- **Dependency:** **the v1.7 additive backend (§3)** for both ticketing features + the gate; **Phase B-0 auth** for testability.

### Phase B-Inbox — Inbox/Conversations enhancements *(app-feature track; PARTIALLY SHIPPED 2026-07-28)*
- **[✅ SHIPPED 2026-07-28 — #3 archive + #4 archived view]** (app `44d382e`, UI refined to `3bfa024`). Delivered OUT of the planned order, on its own, at operator request. **#6 search was already live.** Only `archived_at` was added — **NOT** `deleted_at`, **NOT** `last_read_at` — plus a partial index and the DB trigger `trg_unarchive_on_inbound` (a lead's reply auto-unarchives; must be a trigger because inbound arrives via Edge Functions and would bypass app code). **Delete deliberately skipped** — archive covers the need without risking SMS history. Also shipped alongside: the **lead-type avatar** (icon by acquisition/engagement type instead of a name initial). As-built authority = `/mobile-app` Tab 1. **Still open: #1 (name/email from conversation), #2 (recent/oldest/unread filter), #5 (contact notes)** — #2 still needs the `last_read_at`-vs-localStorage decision.
- **[Original plan — scoped 2026-06-20, parked.]** Full scope + build plan: **`docs/inbox-conversations-enhancements-scope.md`**. Mikey's confirmed 6-feature set (priority-ordered): (1) add name/email to a contact from inside the conversation, (2) filter recent/oldest/**unread**, (3) **delete or archive** a conversation, (4) archived-conversations view, (5) notes per conversation (linked to contact), (6) search by name/number.
- **Schema impact is tiny:** only #2 (unread) + #3 (archive/delete) touch schema. `contacts` already has `first_name/last_name/email/notes/deleted_at`; `conversations` needs **3 additive nullable columns** — `archived_at`, `deleted_at` (**soft-delete**, recommended — hard-delete cascades + destroys SMS history), `last_read_at` (unread marker). One **v1.8-style additive pass** (same contract as v1.7; `audit_tenant_rls()=0` preserved). **Tag = `v1.8` unless Phase D's A2P pass lands first → then `v1.9`.**
- **Structure mirrors B-backend → B-design:** additive backend pass (3 cols + indexes + 5 role-verified server fns reusing `tickets/authz.ts`) → validate → UI (list filter+search, archive/delete, archived view, name+notes edit in-thread, mark-read). **Est. ~2 days** at our cadence (backend ~0.5d, UI ~1–1.5d).
- **Dependency:** B-design closed (don't interleave). Lower-complexity than ticketing (no service-role-only write boundary — these tables are already client-writable; the fns field-scope the edits).

### Phase C — Admin-view + agency-view + the onboarding wizard *(Project-1 admin builds)*
- **The onboarding wizard itself (the big missing build).** `createClient` accepts only slug/business_name/phone_display/email; `onboard-from-form` defines the field→destination mapping but **the capture UI does not exist** (the skill overstates it as "Stage 3 built"). This is the **linchpin of Phase E** — build the wizard + extend `updateClientSettings` to cover all §9b fields. **[IN PROGRESS — C-3, sub-sliced; spec `docs/phase-c-prompt-c3-wizard-ui.md`. Decisions (2026-06-21): Place ID dropped from the wizard (unused at runtime); hours SPLIT — public `clients.hours` (client-filled, shows on site) vs private `send_settings.business_hours` (agency timing, lead-reply branch), both canonical `{"mon":["09:00","17:00"],...}` shape (fixes the `{raw}` bug). App-layer fn tweak: add `hours` to `onboarding.functions.ts` ClientFields.]**
...
- **Gated list:** (1) real outbound TextGrid swap → live SMS; (2) net-new inbound webhooks live; (3) **A2P registration flow** (subaccount→Brand→Campaign→number — the last build piece); (4) bot-shield + rate-limit live — **no keys to provision: Turnstile RETIRED 2026-07-22, replaced by the invisible native PoW shield (one platform-wide `POW_SECRET`)**; (5) **pg_cron scheduling LAST — ✅ DONE 2026-07-29** (`drip-runner`, `*/2 * * * *`, → `app.pierceworks.co/api/public/cron/sequences`); (6) the **live flip** (`SMS_MODE=live`) + the **8-gate LIVE-flip checklist** (HANDOFF §2: parent-on-subaccount auth, form-encoding, HMAC URL-exactness, etc.).
...
- **Testing ORDER (what must be green before real traffic):**
  ...
  3. Remix renders that client's real data (set `VITE_CLIENT_SLUG`, domain connected, `allowed_origins` set). **No Turnstile keys/hostname step — RETIRED 2026-07-22; the PoW shield needs no per-client keys and no hostname registry.**
...
- **What stub/frontend testing did NOT catch (Phase E surfaces it):** live TextGrid auth (parent-on-subaccount 401 risk), form-encoding/content-type, inbound HMAC URL-exactness, CORS with the real domain, A2P carrier approval of the actual site, **consent persistence** actually writing records, owner-email deliverability, and **client login provisioning**.
...
NOW ──► TextGrid verification (external, long pole) ───────────────────────────────►  per-client A2P approval ─► LIVE FLIP
        ...
        ├─ Phase B-design  (client PWA + admin ticket surfaces, app-layer) ─► DONE 2026-06-20 (3 slices validated, no re-tag)
...
| **Per-client domain/DNS + one-time agency sender domain** (NS-delegated, DKIM/SPF) | Phase-E runbook (domain connect, `allowed_origins`, A2P site URL) + one-time agency email-domain setup | per-client + one-time |
| ~~**Real Turnstile keys per client**~~ — **ELIMINATED 2026-07-22** | Turnstile RETIRED; the invisible native PoW shield needs **no per-client keys, no site key, no hostname registry** — just the platform-wide `POW_SECRET`. Nothing to provision per client. | ~~per-client~~ → none |
