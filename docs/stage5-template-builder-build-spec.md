...
- **STYLE** = a selectable PRESET (copy voice + photo selection/treatment) that fills the shell and gives it a feel. Set of 5: **Family-Owned, Owner-Operated, Corporate/Professional, Modern Professional, Local Professional.** Reusable, NOT client-specific.
...
- **`site_style` = the template/style SELECTION key, NOT a render-time branch** (the v1 [FIX] stands). `site_style` is free text (not enum), so revising the value set to the 5 presets is convention/UI only — **no migration.**
...
- **[FIX — doc reconciles]** `workspace-knowledge-condensed` "Project 2 singular" → N style templates; website-structure 4 styles → the 5 style presets (copy voice + photo treatment) + niche decoupled (data-only, `site_style` free text — no migration); a2p §C /review wording relaxed.
