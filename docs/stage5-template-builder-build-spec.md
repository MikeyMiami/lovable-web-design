...
- **[BACKLOG — from the Professional Modern build audit, 2026-06-18; FUTURE, not this build]:**
  - **(A-3 / G-3) Consent persistence.** `/a2p-site-compliance` mandates the two-checkbox opt-in, but the frozen `intake`/`discount` routes have **no consent field** — the checkboxes are **display-only, never recorded**, while the Privacy Policy copy claims "timestamped records of all opt-in actions." Decide persist-vs-display; if persist → **additive backend fields + route change (post-freeze, re-validate + re-tag)** — its own spec, NOT a template build. (Carrier review checks the on-site opt-in *displays*, so v1 templates are unaffected.)
  - **(C-3 / G-4) Terms/privacy link-token canonicalization.** Two keys exist for the same on-site links: `website_terms_page_link` (opt-in-forms/template-builder/onboard) vs `{privacy_url}`/`{terms_url}`/`{optin_url}` (a2p token schema) → footer Terms vs consent Terms can diverge. Define `privacy_url`=`/privacy`, `terms_url`=`/terms`, `optin_url`=the opt-in page; reconcile `website_terms_page_link` → `/terms` (alias or retire). Skills/docs only when done.
