...
## I. Copy system
- `default-copy.ts`: exactly the 10 slots (the 8 prose slots + `home.call_bar_label` / `home.cta_button`, whose defaults are LITERAL "Call Now" / "Chat with us now" — assert they interpolate no client data); resolver = content-override → buildDefault, no raw tier. `grep -rn "about_us" src/routes/ src/components/site/` → only as buildDefault INPUT, never rendered raw.
- **Call-Now button + hero chat CTA** (both required since 2026-07-27): `grep -rn "CallNowBar" src/components/site/` → imported and rendered inside `Header.tsx` (centered between logo and nav); it is NOT expected in `PageShell`/`HeroShell` (placement revised 2026-07-27). **Then check mobile:** `<CallNowBar />` must render in the header bar with NO `hidden md:` wrapper and must NOT appear inside the `{open && …}` mobile-menu block — `grep -c CallNowBar Header.tsx` = exactly one render site. Either a `hidden md:` wrapper (invisible on phones) or a mobile-menu copy (buried behind the hamburger, and it double-renders) = finding. **Also check `styles.css` has a dark `html { background-color }` and a matching `theme-color` meta** — without it iOS paints a white bar behind the status bar that stays pinned while scrolling (see `/template-builder` §8b); and confirm `viewport-fit=cover` is NOT set unless safe-area insets are. Also confirm the visible number passes through `formatPhoneUS`. `grep -n "tel:" src/components/site/CallNowBar.tsx` → href built from `phone_e164` ONLY (a `tel:` built from `phone_display` = finding); a `return null` guard on blank `phone_e164` is present; no hardcoded digits in the file. `grep -rn "open-chat-widget" src/` → exactly TWO sites: the `addEventListener` in `ChatWidget.tsx` and the `dispatchEvent` in `index.tsx`. The hero button is gated on `chat_widget_enabled !== false`, is `type="button"`, and `ChatWidget`'s form/submit/bot-shield code is unchanged (a duplicated chat form behind the CTA = finding).
