...
> **⚠️ HISTORICAL BUILD PLAN — point-in-time record, NOT current state.** Two things below are superseded. (1) **The messaging profile and TeXML app are NOT per-client.** Since 2026-07-29 there is ONE shared **"Pierceworks" messaging profile** (`40019f80-6b8f-473f-afdd-23a7f32151c5`) and ONE shared **"Pierceworks Voice" TeXML app** (`3008335895855105762`); `telnyx_messaging_profile_id` and `telnyx_texml_app_id` were removed from Settings, onboarding and the go-live checklist (columns kept, no migration). Only `telnyx_number` / `telnyx_brand_id` / `telnyx_campaign_id` are per client. (2) **TextGrid is frozen legacy**, not a live parallel path — Telnyx is the only provider offered to new clients. **Current authority = the `/telnyx-provider` skill + `docs/ONBOARDING-ORDER-OF-OPERATIONS.md` Phase C.**
...
| D10 | ~~**Telnyx auto opt-out DISABLED on the messaging profile**~~ **SUPERSEDED 2026-07-29 — not achievable:** Telnyx auto opt-out is default behavior and cannot be fully disabled (docs-verified); blocks are MESSAGING-PROFILE-WIDE. Real architecture = two layers (Telnyx blocks + auto-confirms; app exits drips off the still-delivered webhook + owns `pass`). See /telnyx-provider "Opt-out ownership". | Corrected by 2026-07-29 audit |
...
- **J5 — Opt-out ownership:** ~~confirm D10 (disable Telnyx auto-opt-out; our app stays the single source of truth)~~ **RESOLVED 2026-07-29: D10 impossible — auto opt-out cannot be disabled; two-layer model adopted (see /telnyx-provider).**
