...
G6. Hero chat CTA: a real <button type="button"> at the bottom of the hero text block; label = resolveCopy("home.cta_button"); onClick dispatches the "open-chat-widget" CustomEvent; rendered ONLY when client.chat_widget_enabled !== false; does not submit or navigate; the H1 text and source are unchanged.
