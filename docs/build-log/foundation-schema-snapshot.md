...
>
> **Validation verdict:** PASS on all locked invariants. Open items at the bottom (review_gate_mode enum mismatch vs `/admin-view`; anon column-scope (RESOLVED 2026-06-14 — see item #2); helper-EXECUTE runtime verify; RLS-audit gate TODO tracked in spec §12).
...
2. ~~**anon `clients` SELECT is row-level only** — exposes all 27 columns to anon.~~ **RESOLVED 2026-06-14 (Stage 3.5 §A) — live-verified.** As-built: REVOKE table-wide anon SELECT; GRANT anon SELECT on the **13 presentational columns** (`slug, business_name, tagline, phone_display, address, hours, license_number, logo_url, brand_color, service_area, social_links, template_vars, review_link`) **+ `status` + `deleted_at`** (for the `security_invoker` view's WHERE); KEEP the status-filtered row policy; `security_invoker` `clients_public` view over those columns (linter ERRORs on SECURITY DEFINER → invoker → column-grants → full revoke/view-only impossible). `quote_form_link` lives in the granted `template_vars` jsonb (not a top-level column → set is 13, not 14). Proven by LIVE anon queries: anon→`clients_public` returns data; sensitive (`email`/`twilio_number`/`call_forwarding_number`) = 42501 denied; `audit_tenant_rls()`=0. *(An earlier GRANT-didn't-land bug was caught by the confirm queries — not the report's checkmarks — and fixed.)* Companion finding `storage_objects_no_policies` also fixed (3 policies per `scratch-foundation` §9). See `stage-3.5a-security-validation.md`.
...
4. ~~**RLS-audit gate (guardrail 1)**~~ — **RESOLVED (2026-06-09):** `audit_tenant_rls()` built + PASSES (0 violations). Record: `guardrail-1-rls-audit.md`. (Non-blocking enhancements noted there: gate `clients` too; add functional isolation test.)
5. ~~**`sending_subdomain` / `dkim_status` NOT built**~~ — **RESOLVED (2026-06-09):** the 3 skills (`scratch-foundation` §1, `admin-view`, `onboard-from-form`) updated from "columns exist (deferred)" → "NOT created in v1 (deferred; `ADD COLUMN` later if per-client email is built)." Skill text now matches the as-built 27-col `clients` table. No migration.
