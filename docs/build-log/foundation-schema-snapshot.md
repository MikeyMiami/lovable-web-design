...
2. ~~**anon `clients` SELECT is row-level only** — exposes all 27 columns to anon.~~ **RESOLVED 2026-06-14 (Stage 3.5 §A):** anon table-wide SELECT REVOKED; column-GRANTs to anon on ONLY the 14 presentational columns (`slug, business_name, tagline, phone_display, address, hours, license_number, logo, brand_color, service_area, social_links, template_vars, review_link, quote_form_link`); status-filtered anon row policy KEPT; a **`security_invoker`** `clients_public` view added over those columns (the linter ERRORs on SECURITY DEFINER views → invoker required → column-grants required → full revoke/view-only NOT possible). Sensitive columns (`twilio_*`, `call_forwarding_number`, `email`, `allowed_origins`) denied to anon. Both critical scan findings cleared; `audit_tenant_rls()`=0. **Residual confirms** (see `stage-3.5a-security-validation.md`): grant list = exactly the 14; the retained anon row policy keeps `status='active' AND deleted_at IS NULL` (status-filter on direct base reads). Companion finding `storage_objects_no_policies` also fixed (3 policies per `scratch-foundation` §9).
