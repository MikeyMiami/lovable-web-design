...
>
> **Validation verdict:** PASS on all locked invariants. Open items at the bottom (review_gate_mode enum mismatch vs `/admin-view`; anon column-scope (FIX IN PROGRESS 2026-06-14 — see item #2); helper-EXECUTE runtime verify; RLS-audit gate TODO tracked in spec §12).
...
2. **anon `clients` SELECT — FIX IN PROGRESS (Stage 3.5 §A).** Was row-level only → all 27 columns exposed. Plan/mechanism: REVOKE table-wide anon SELECT + GRANT only the 14 presentational columns + KEEP status-filtered row policy + a `security_invoker` `clients_public` view (the linter ERRORs on SECURITY DEFINER views → invoker required → column-grants required → full revoke/view-only NOT possible). **BUG found by the §A confirm queries (2026-06-14): the REVOKE landed but the GRANT did NOT — anon has 0 base-`clients` grants, so the `security_invoker` view permission-denies for anon (marketing site can't read).** Row policy ✅ (status filter `status='active' AND deleted_at IS NULL` retained); `audit_tenant_rls()`=0 ✅. Re-GRANT of the 14 columns in flight (+ pin `logo` vs `logo_url` real name so view + grant agree — suspected GRANT root cause). Companion finding `storage_objects_no_policies` IS fixed (3 policies per `scratch-foundation` §9). **NOT closed until anon can actually SELECT through `clients_public` and get data + sensitive denied + audit 0.** See `stage-3.5a-security-validation.md`.
