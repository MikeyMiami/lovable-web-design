...
2. ~~**`revokeUserRole` authorization UNVERIFIED**~~ — **RESOLVED (2026-06-09):** code reviewed, authorization matrix CONFIRMED symmetric with assign (platform-admin revoke any; client_owner own-client_id only + cannot revoke admin/agency_owner/client_owner; else Forbidden; delete scoped `client_id=X`/`IS NULL`). **Note:** revoke writes NO audit row (even tenant-scoped) — so the audit_log TODO (#3) must cover **grants AND revokes, both scopes**. Minor: no self-lockout guard (last-admin) — low-priority hardening before admin UI.
