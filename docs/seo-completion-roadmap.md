...
- **Change type:** mixed. **LOC-1** = prompt/JSON-only (add `locations[]` to `template_vars.seo`, AI-seed from `service_area` + curate). **Also make the PRIMARY/home city EXPLICIT here** — add a `city` to `template_vars.seo` (the GBP/home city) so `seedCoreThirty`/`aiWritePage` anchor the homepage + Core-30 to the *business's* city rather than relying on `service_area[0]` being home-first (today's resolution `seo.city ?? service_area[0] ?? address` can pick the wrong city if the owner didn't list home first). The homepage H1 stays `{primary category} + {city}` (`40-page:102-108`) anchored to this primary city; geo pages (LOC-2) cover the *other* serviceable areas. **LOC-2** = **schema-additive** (`content_pages.geo_area text`, additive; passes audit) + `seedGeoPages` (draft `type='geo'` rows per selected location/service) + a **coverage matrix** (services × locations) + **batch-generate** (Worker-safe sequential). **LOC-3** = geo-aware `aiWritePage` (reads `geo_area` for local context; genuinely-local prompt) + **publish-in-waves**.
