...
### Slice 3 — Multi-location (LOC-1 → LOC-2 → LOC-3) — IN PROGRESS [biggest gap]
...
- **Change type:** mixed. **LOC-1 (done)** = JSON-only, as above. **LOC-2 (next)** = **schema-additive** (`content_pages.area_served text`, additive; passes audit — named `area_served` to match the template's existing `page.area_served` consumer + schema.org, and the anon RPCs `select cp.*` so it auto-flows with **zero RPC/template change**) + `seedGeoPages` (draft `type='geo'` rows keyed by the frozen **location slug**) + a **coverage matrix** (default primary-category × each location; operator can add cells) + **batch-generate** + the **deletion/cascade guard**. **seedGeoPages SKIPS the primary city** (home already covers it — no double-targeting). **LOC-3** = geo-aware `aiWritePage` (reads `area_served` for local context; genuinely-local prompt) + **publish-in-waves**.
...
| 3 | multi-location (LOC-1/2/3 + geo/supporting creation) | mixed (LOC-2 additive) | IN PROGRESS (LOC-1/1b/1b-final DONE; LOC-2 next) |
