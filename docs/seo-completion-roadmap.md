...
| SVC-5 | Assets tab — shared `AssetPool` (view by category incl. `by_service` + filter) + post-onboarding upload/delete (`addSiteAsset`/`deleteSiteAsset` read-merge-write → public-assets) reused in the Photo-Board | additive UI + fns | PLANNED (next — last SVC slice). Scope: `docs/phase-structured-services-scope.md` |
