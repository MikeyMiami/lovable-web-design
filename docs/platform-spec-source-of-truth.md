...
> ⚠️ **SUPERSEDED 2026-07-16 — CAPTURE-FIRST, NO AI.** The `/chat-widget` skill is now authoritative; the AI-Q&A description below is historical. The widget is a **lead form in a chat skin**: bubble → customizable greeting → First/Last/Phone (+ message) + consent + Turnstile → `/api/public/chat/optin` → contact `source='chat_widget'` + `channel='chat'` message → the SAME lead-form drip → owner notification "New Website Chat Widget Lead". The AI streaming/FAQ path (stream.ts, knowledge.server.ts, gemini gateway) is built-but-PARKED for a possible v2 toggle. Read `/chat-widget`, not the paragraph below.
...
- **Bot protection** — **SUPERSEDED 2026-07-22: Cloudflare Turnstile RETIRED, replaced by an invisible native PoW (proof-of-work) shield.** Forms mint a challenge from `/api/public/challenge`, solve it in a Web Worker, and submit a single-use `pow_token` plus a hidden `website` honeypot. **No third-party widget, no site key, no secret per client, and no hostname registry** — one platform-wide `POW_SECRET`, unlimited client domains at $0 (chosen over Turnstile's free-tier ~200-domain cap). Backend is fail-CLOSED: a form without the shield = zero leads. *(Original entry: Turnstile site key in forms + secret verified server-side, each client domain added as a hostname.)*
