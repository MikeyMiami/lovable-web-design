...
- [PRE-FREEZE CLEANUP — Stage 3.5] A dedicated cleanup stage runs AFTER the Stage-3 tabs and BEFORE the Stage-4 freeze, batching the deferred non-Twilio items so the frozen master is complete: (1) **audit_log** table (role-mutation audit — see the [BUILD — TODO] above), (2) **export-client** server fn / isolation guardrail 3 (see the [BUILD — TODO] above; `/launch-check` §A), (3) the **3a send-primitive regression re-test** (the bullet directly above; `/launch-check` §B). These are not Twilio-dependent (so NOT 1f) — they're the pre-freeze finishing batch, all gated within `/launch-check` A–D. Genuinely Twilio-dependent work (real Twilio swap, Turnstile/rate-limit, real-time reply-driven exits, the materialization status-swap) stays at 1f.
