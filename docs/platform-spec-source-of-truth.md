...
**Trigger:** inbound call to the client's provider (TextGrid) number ends with a call status of **`busy`, `no-answer`, `canceled`, or `failed`** (the Twilio-API-compatible literal status strings TextGrid returns — note `no-answer` hyphenated, `canceled` one L). Voice-status webhook (`/api/public/twilio/voice-status`). Note: a call that rolls to voicemail reports as `completed` and does NOT fire (catching voicemail would require Answering Machine Detection — out of scope).
