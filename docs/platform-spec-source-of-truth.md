...
- [BUILD — 1f] Runner → `messages`/`conversations` materialization (3b finding): today the cron-runner stub logs `sms_sent` to `events` only and does NOT write `messages`/`conversations`; only the reply path inserts. So drip-sent SMS are ABSENT from the Conversations inbox, and a missed-call Open-conversation deeplink lands on an empty thread (no materialized outbound). At 1f (real Twilio) the runner must write each outbound text into `messages` + bump `conversations.last_message_at`, AND extract a shared `insertOutboundMessage({clientId,contactId,body,sid,status})` helper — **admin variant for cron (service-role) + RLS variant for reply (authed)** — reused by the runner + `reply.functions.ts`; keep the send primitive SEND-ONLY (materialization in the callers). This is MORE than swapping the stub for Twilio (a runner logic change + reply-path refactor). Mirrored in `/launch-check` §D. *(Open Q: this is conversation LOGIC — consider doing it PRE-freeze so the frozen master's inbox is complete + the helper is proven; see `docs/build-log/stage-3b-validation.md`.)*
- [PRE-FREEZE — Stage 4] Send-primitive extraction regression (from 3a): `sendStubSmsWithRetry` was extracted to `lib/sms/send.server.ts` and is now shared by the cron runner + the reply path. **Before the Stage-4 freeze:** re-run the 2e TEST1–TEST5 drip walks to confirm the runner's claim-lease / window-gating / caps / reschedule-without-advancing / advance-on-success / 2× retry are intact after the stub→shared-primitive extraction, AND confirm the primitive is SEND-ONLY (no DB writes / no cron-decision event-logging leaked in — else the reply path inherits it). Tracked as a checkbox in `/launch-check` §B.
