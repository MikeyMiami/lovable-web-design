...
> Phone: {phone}
> Email: {email}
- **Inbound SMS → CRM** [1f — NET-NEW, not built] — the inbound webhook (provider = TextGrid; `X-TextGrid-Signature` HMAC-SHA1 verified, under `/api/public/*`) creates/updates conversation + message + drives reply-exits / missed-call textback; STOP/HELP/START native at the provider + `pass` handled at the webhook. *(Verified 2026-06-16 against the frozen repo: NO inbound route or signature check exists — this whole layer is 1f net-new; the earlier "[built]" was inaccurate.)*
...
- **Secrets:** the agency **master account** auth token is the single platform runtime secret (never on a row). Per-client subaccount creds are the additive 1f columns (`provider_subaccount_sid`, `provider_webhook_secret`) — the webhook secret backs the `X-TextGrid-Signature` verification below.
...
- **Inbound + voice-status webhooks [1f — NET-NEW, not a swap]:** per-client subaccount webhooks → `/api/public/*`; route to the correct client by the `To` number → `clients` row; **`X-TextGrid-Signature` (HMAC-SHA1) verified** (per-client `provider_webhook_secret`) BEFORE any DB write. *(No inbound route or signature check exists in the frozen master — built fresh at 1f.)*
