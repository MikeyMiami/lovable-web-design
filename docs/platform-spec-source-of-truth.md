...
- [STAGE 3.5 §A — SECURITY — APPLIED 2026-06-14, confirms pending] `clients_anon_select_sensitive_fields` (= foundation open-item #2): anon had table-wide SELECT on `clients` → all 27 columns exposed (the row policy was status-filtered, NOT column-scoped). FIX (as-built): **REVOKE anon's table-wide SELECT; GRANT SELECT to anon on ONLY the 14 presentational columns** (`slug, business_name, tagline, phone_display, address, hours, license_number, logo, brand_color, service_area, social_links, template_vars, review_link, quote_form_link`); **KEEP the status-filtered anon row policy** (`status='active' AND deleted_at IS NULL`); expose a **`security_invoker` `clients_public` view** over those columns; repoint the marketing data-loader at the view. Exclude ALL `twilio_*` + `call_forwarding_number` + `email` + `allowed_origins`; site renders the number from `phone_display`. **NOTE: a `security_invoker` view is required (the linter ERRORs on SECURITY DEFINER views) and needs anon to retain column-scoped base access — so full anon-revoke / view-only is NOT possible; column-grants + the retained status row-policy is the correct shape.** Confirms pending: (1) grant list = exactly the 14; (2) the retained anon row policy keeps the status filter (so direct base reads can't see inactive clients). `/launch-check` §A; see `stage-3.5a-security-validation.md`.
