...
- **Code ownership/reproducibility:** connect Project 1 to a GitHub repo (two-way sync) + download codebase = the golden master is owned and reproducible (within Lovable via Remix → migrations re-run; outside via export + own Supabase). Remix carries code + migrations, NOT secrets/data/integrations/domain (re-added per project).
