...
- [VERIFY — TODO] revokeUserRole authorization (1d): confirm same matrix as assignUserRole — client_owner cannot revoke admin/agency_owner/client_owner; scoped to own client_id; platform-admin (admin/agency_owner) can revoke any. Code not yet reviewed.
...
- [BACKLOG — IDEA] Read-only admin debug/research interface: authenticated endpoint(s) for an agency-owner/admin (RLS + `is_admin` legitimately spans all clients) — **READ-ONLY** cross-data debugging/analysis, primarily over the `events` audit trail + enrollment/message state, for programmatic analysis by an external tool. Rules: NEVER expose the service-role key to the caller; SELECT-only; prefer aggregated/metadata over raw PII (privacy); overlaps with the export-client fn (guardrail 3) — generalize that pattern. Repo is public → keys stay runtime secrets. Owner: post-foundation tooling.
