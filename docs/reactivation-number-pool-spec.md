...
> **🛑 SUPERSEDED — 2026-07-21: THE AGENCY NUMBER POOL WAS REMOVED.** Reactivation now sends from the **client's OWN Telnyx number** (`clients.telnyx_number`, `provider='telnyx'`) via the **per-client `reactivation` drip delivered by the NORMAL send runner** — inheriting the normal throttles (`daily_send_cap`, per-client slice, jitter, send-window) + the reactivation enrollment caps (50/day, 2 per 20 min, dedup) + a **REQUIRED consent attestation** ("past customers who consented to SMS"). The entire pool was DELETED: the `reactivation_numbers` / `reactivation_campaigns` / `reactivation_enrollments` tables, the `create_reactivation_campaign` / `claim_due_reactivation` RPCs, the finite-campaign runner + `/api/public/cron/reactivation` route, and the "Number Pool" admin surface. (The global `reactivation` SEQUENCE remains — the per-client path uses it.) **This doc is retained as HISTORICAL RATIONALE only.** Its reputational-isolation reasoning is exactly what informs a deliberately **NOT-built future seam**: an OPTIONAL per-client dedicated `telnyx_reactivation_number` (a one-line from-resolution change) could later re-isolate a cold list from the client's standing number. v1 instead relies on throttling + the required consent + honoring STOP. **The blockquote immediately below — which claimed this pool supersedes the per-client drip — is itself now reversed.**
