...
- **Onboarding lifecycle:** 7-step wizard (agency `/onboard` or one-time tokenized client link; collects identity, content, discount offer, support/contact emails, A2P identity + optional CP 575 letter upload) → `status='pending'` → agency `/admin/review` → **Finalize & Invite** (flips `active`, invites `client_owner`) or **Decline** (admin fn, pending-only guard = PERMANENT purge of row + storage).
