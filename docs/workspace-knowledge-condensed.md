...
- **Project structure:** Project 1 = shared backend + admin dashboard + tenant app (`app.theirdomain.com`); owns the DB. **N STYLE templates** (one frontend-only Lovable project per style preset — Professional Modern, Artistic Unique, Corporate, Modern Tech, Family Owned, Owner Operated; niche is a decoupled DATA layer, not a separate project). Per-client sites = Remixes of the chosen style template, `.env` → Project 1's Supabase. Each style template **bakes in the a2p compliance surface** (single-checkbox opt-in, named Privacy/ToS, SMS Program page, footer links, `/review`) **+ the Turnstile widget**, so every remix is compliant by construction; niche fills `template_vars.segment` + the two consent-category strings from the `/a2p-site-compliance` library. (Layer model: `/template-builder` + `docs/stage5-template-builder-build-spec.md`.)
...
- **Messaging provider = TextGrid** (Twilio-API clone; fetch, NO SDK): agency master account → **per-client subaccount → Brand (client EIN) → Campaign → number**; each vets independently per-client (~2–4 days). Per-client From/MessagingServiceSid (non-secret, on `clients`, column names retained); master auth token = runtime secret; per-client webhook secret + subaccount SID/auth token in the server-only `client_provider_secrets` table (2026-07-22 — moved off clients, columns dropped) for webhook verification. **Inbound + voice-status webhooks are NET-NEW at 1f** (do NOT exist in the frozen master): per-client subaccount routes under `/api/public/*`, route by `To`, verify **`X-TextGrid-Signature`** BEFORE any DB write. Outbound `from` is caller-resolved; the send primitive stays SEND-ONLY.
...
- **Bot protection [2026-07-22 — native PoW shield primary; legacy Turnstile verified during transition]:** self-hosted stateless HMAC challenges via `/api/public/challenge`, solved client-side in a Web Worker → `pow_token` (sig + 3s–10min age window + difficulty + one-time replay via the rate-limit RPC) + hidden `website` honeypot (non-empty → silent fake-success, nothing created). `POW_SECRET` Cloud secret required (unset = fail-OPEN + alert). Enforced server-side **fail-CLOSED** on the 3 public lead-intake routes (`intake`/`discount`/`chat/optin`); **fail-OPEN+alert** on a siteverify infra failure; `chat/request` = rate-limit-only; webhooks/cron excluded (1f step 3, shipped). The shield is **baked into the marketing template** — no shield = no token = zero leads; NO per-client hostname registry, no scale cap, $0 at any client count.
...
- **Drips:** Review Request (4 SMS), One-Year Follow-Up (5 SMS, exit on reply/opt-out), Lead-Form (business-hours branched), Missed-Call Textback, Reactivation. **Reactivation sends from the CLIENT'S OWN Telnyx number** (`clients.telnyx_number`, `provider='telnyx'`) via the per-client drip delivered by the NORMAL runner — normal throttles + the reactivation enrollment caps (50/day, 2/20min, dedup) + a REQUIRED consent attestation; "Upload Customers" CSV, same 4 texts as review. **The former AGENCY number-POOL model was REMOVED 2026-07-21** (its `reactivation_*` tables, `create_reactivation_campaign`/`claim_due_reactivation` RPCs, finite-campaign runner + `/cron/reactivation`, and "Number Pool" admin surface are deleted; the global `reactivation` SEQUENCE remains — the per-client path uses it).
